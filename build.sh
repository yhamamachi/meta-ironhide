...
if [[ "$(cat ${SCRIPT_DIR}/conf/layer.conf 2>&1 | grep LAYERDEPENDS_${MACHINE} )" != "" ]] ;then
    IS_BUILD_INSIDE_REPO=yes
else
...
if [[ "${IS_BUILD_INSIDE_REPO}" == "yes" ]]; then
    ...
    git clone https://github.com/yhamamachi/meta-${MACHINE}.git
fi
...
TEMPLATECONF=${WORK}/meta-${MACHINE}/conf/templates/${MACHINE}${TEMPLATE_POSTFIX} \
